...
namespace jw::graphics
{
	...
	bool ConstantBuffer::Create(size_t size)
	{
		...
		jw::graphics::GetDevice()->CreateBuffer(buffer.GetAddressOf(), &desc, nullptr);
	...
	{
		jw::graphics::GetDevice()->SetConstantBuffer(buffer.Get(), data, desc.ByteWidth);
	}

	void ConstantBuffer::Bind(eShaderStage stage)
	{
		jw::graphics::GetDevice()->BindConstantBuffer(stage, mType, buffer.Get());
	}
