...
namespace jw
{
	StudioScene_Boss::StudioScene_Boss()
	{
		SetName(L"StudioScene_Boss");
	}
	StudioScene_Boss::~StudioScene_Boss()
	{
	}
	void StudioScene_Boss::Initialize()
	{
		//UI
		{
			// hud_base
		...
		//bg
		{
			...
			mr->SetMesh(Resources::Find<Mesh>(L"RectMesh"));
			mr->SetMaterial(Resources::Find<Material>(L"SpriteMaterial_room_haunted_finale2_boss"));
			player->GetComponent<Transform>()->SetPosition(Vector3(0.0f, 0.0f, 0.0f));
			player->GetComponent<Transform>()->SetScale(Vector3(10.0f, 5.5f, 1.0f));
		}
	...
	void StudioScene_Boss::Update()
	{
		Scene::Update();

		if (Input::GetKeyDown(eKeyCode::N))
		{
			SceneManager::LoadScene(L"BunkerScene_00");
		}
	}

	void StudioScene_Boss::LateUpdate()
	{
		Scene::LateUpdate();
	}

	void StudioScene_Boss::Render()
	{
		Scene::Render();
	}
}