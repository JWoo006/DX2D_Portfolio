...
namespace jw
{
	FactoryScene_00::FactoryScene_00()
	{
...

		{
			...
			player->GetComponent<Transform>()->SetScale(Vector3(32.0f, 5.0f, 1.0f));
		}
		{
			GameObject* player = new GameObject();
			...
			MeshRenderer* mr = player->AddComponent<MeshRenderer>();
			mr->SetMesh(Resources::Find<Mesh>(L"RectMesh"));
			mr->SetMaterial(Resources::Find<Material>(L"SM_spr_idle_0"));
			player->GetComponent<Transform>()->SetPosition(Vector3(0.0f, -1.0f, -1.0f));
			player->GetComponent<Transform>()->SetScale(Vector3(0.5f, 0.5f, 1.0f));
		}
		...
		//Main Camera
		{
			...
			camera->GetComponent<Transform>()->SetPosition(Vector3(-0.8f, -0.5f, -10.0f));
